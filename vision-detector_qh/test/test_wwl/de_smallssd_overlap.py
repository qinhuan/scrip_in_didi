...
def non_max_suppression_fast(x1, y1, x2, y2, conf, overlapThresh):
    ...
    while len(idxs) > 0:
        ...
        #w = np.maximum(0, xx2 - xx1 + 1)
        #h = np.maximum(0, yy2 - yy1 + 1)
        #idxs = np.delete(idxs, np.where(overlap2 > 0.8)[0])
        xx1 = np.maximum(x1[i], x1[idxs[:last]])
        yy1 = np.maximum(y1[i], y1[idxs[:last]])
        xx2 = np.minimum(x2[i], x2[idxs[:last]])
        yy2 = np.minimum(y2[i], y2[idxs[:last]])
...
if __name__ == '__main__':
    ...
    model_def = '/home/work/qinhuan/git/vision-detector/test/test_wwl/toqinhuan/smallssd/deploy.prototxt'
    model_weights = '/home/work/qinhuan/git/vision-detector/test/test_wwl/toqinhuan/smallssd/final.caffemodel'
    ...
    fout = open('/home/work/qinhuan/git/vision-detector/test/test_wwl/toqinhuan/txts/res_smallssd_0.9_andoverlap1_0.9.txt', 'w')
