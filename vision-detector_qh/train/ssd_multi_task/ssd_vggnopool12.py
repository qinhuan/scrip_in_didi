...
def create_inputs(net, phase, Params):
    global batch_sampler
    if phase == 'train':
...
def create_multibox_loss(net, mbox_layers, loss_weight=1.0):
    return L.MultiBoxLoss(*mbox_layers, 
            multibox_loss_param=multibox_loss_param.copy(),
            loss_param=loss_param, 
            include=dict(phase=caffe_pb2.Phase.Value('TRAIN')),
            propagate_down=[True, True, False, False], 
            loss_weight=loss_weight)

def create_multibox_outputs(net, mbox_layers, label_layer,
        conf_name = "mbox_conf", det_out_param=det_out_param, 
        det_eval_param=det_eval_param):
    if multibox_loss_param["conf_loss_type"] == P.MultiBoxLoss.SOFTMAX:
        reshape_name = "{}_reshape".format(conf_name)
        net[reshape_name] = L.Reshape(net[conf_name], 
            shape=dict(dim=[0, -1, det_out_param['num_classes']]))
    ...
    detection_out = L.DetectionOutput(*mbox_layers,
        detection_output_param=det_out_param,
        include=dict(phase=caffe_pb2.Phase.Value('TEST')))
    detection_eval = L.DetectionEvaluate(detection_out, net[label_layer],
        detection_evaluate_param=det_eval_param,
        include=dict(phase=caffe_pb2.Phase.Value('TEST')))

    return detection_out, detection_eval

def proto_index(proto, name):
...
def create_net(phase): 
    ...
    # layers unused
    garbage_layers = []
    net.data_lisa, net.label_lisa = create_inputs(net, phase, Params['LISA'])
    net.data_kitti, net.label_kitti = create_inputs(net, phase, Params['KITTI'])
    
    net.data = L.Concat(net.data_lisa, net.data_kitti, axis=0)
    
    mbox_source_layers = create_body_net(net, from_layer='data')
    Params['num_classes'] = Params['LISA']['num_classes'] + Params['KITTI']['num_classes']
    mbox_layers = create_multibox_layers(net, mbox_source_layers)
   
    # slice lisa and kitti
    if phase == 'deploy':
        net['mbox_conf_reshape'] = L.Reshape(net['mbox_conf'], 
                shape=dict(dim=[0, -1, Params['num_classes']]))
        net['mbox_conf_lisa'], net['mbox_conf_kitti'] = L.Slice(
                net['mbox_conf_reshape'], 
                slice_point=Params['LISA']['num_classes'], 
                axis=2, 
                ntop=2)
        mbox_layers_lisa = [net['mbox_loc'], net['mbox_conf_lisa'], net['mbox_priorbox']]
        mbox_layers_kitti = [net['mbox_loc'], net['mbox_conf_kitti'], net['mbox_priorbox']]
    else:
        mbox_layers_lisa = []
        mbox_layers_kitti = []
        name = 'mbox_loc'
        net[name + '_lisa'], net[name + '_kitti'] = L.Slice(net[name], 
                slice_point=Params['LISA']['batch_size_per_device'],
                axis=0,
                ntop=2)
        mbox_layers_lisa.append(net[name + '_lisa'])
        mbox_layers_kitti.append(net[name + '_kitti']) 
       
        name = 'mbox_conf'
        net[name + '_reshape'] = L.Reshape(net[name], 
                shape=dict(dim=[0, -1, Params['num_classes']]))
        net[name + '_lisa_s1'], net[name + '_kitti_s1'] = L.Slice(
                net[name + '_reshape'], 
                slice_point=Params['LISA']['num_classes'], 
                axis=2, 
                ntop=2)
        net[name + '_lisa_s2'], net[name + '_lisa_garbage'] = L.Slice(
                net[name + '_lisa_s1'], 
                slice_point=Params['LISA']['batch_size_per_device'],
                axis=0,
                ntop=2)
        net[name + '_lisa'] = L.Flatten(net[name + '_lisa_s2'], axis=1) 
        garbage_layers.append(net[name + '_lisa_garbage'])
       
        net[name + '_kitti_garbage'], net[name + '_kitti_s2'] = L.Slice(
                net[name + '_kitti_s1'], 
                slice_point=Params['LISA']['batch_size_per_device'],
                axis=0,
                ntop=2)
        net[name + '_kitti'] = L.Flatten(net[name + '_kitti_s2'], axis=1) 
        garbage_layers.append(net[name + '_kitti_garbage'])
        mbox_layers_lisa.append(net[name + '_lisa'])
        mbox_layers_kitti.append(net[name + '_kitti']) 
                
        name = 'mbox_priorbox'
        mbox_layers_lisa.append(net[name])
        mbox_layers_kitti.append(net[name])

    if phase == 'train':
        mbox_layers_lisa.append(net.label_lisa)
        mbox_layers_kitti.append(net.label_kitti)
        multibox_loss_param['num_classes'] = Params['LISA']['num_classes']
        multibox_loss_param['background_label_id'] = Params['LISA']['background_label_id']
        net.mbox_loss_lisa = create_multibox_loss(net, mbox_layers_lisa, 
                loss_weight=Params['LISA']['loss_weight'])
        multibox_loss_param['num_classes'] = Params['KITTI']['num_classes']
        multibox_loss_param['background_label_id'] = Params['KITTI']['background_label_id']
        net.mbox_loss_kitti = create_multibox_loss(net, mbox_layers_kitti,
                loss_weight=Params['KITTI']['loss_weight'])
    else:
        det_out_param_lisa = det_out_param.copy()
        det_out_param_lisa['save_output_param'] = det_out_param['save_output_param'].copy()
        det_eval_param_lisa = det_eval_param.copy()
        det_out_param_lisa['num_classes'] = Params['LISA']['num_classes']
        det_out_param_lisa['background_label_id'] = Params['LISA']['background_label_id']
        det_out_param_lisa['save_output_param']['label_map_file'] = Params['LISA']['label_map_file']
        det_out_param_lisa['save_output_param']['name_size_file'] = Params['LISA']['name_size_file']
        det_out_param_lisa['save_output_param']['num_test_image'] = Params['LISA']['num_test_image']
        det_eval_param_lisa['num_classes'] = Params['LISA']['num_classes']
        det_eval_param_lisa['background_label_id'] = Params['LISA']['background_label_id']
        det_eval_param_lisa['name_size_file'] = Params['LISA']['name_size_file']
        if phase == 'deploy':
            det_out_param_lisa['save_output_param'] = {}

        net.detection_out_lisa, net.detection_eval_lisa = create_multibox_outputs(
                net, mbox_layers_lisa, 'label_lisa', conf_name='mbox_conf_lisa',
                det_out_param=det_out_param_lisa, 
                det_eval_param=det_eval_param_lisa)
       
        det_out_param_kitti = det_out_param.copy()
        det_out_param_kitti['save_output_param'] = det_out_param['save_output_param'].copy()
        det_eval_param_kitti = det_eval_param.copy()
        det_out_param_kitti['num_classes'] = Params['KITTI']['num_classes']
        det_out_param_kitti['background_label_id'] = Params['KITTI']['background_label_id']
        det_out_param_kitti['save_output_param']['label_map_file'] = Params['KITTI']['label_map_file']
        det_out_param_kitti['save_output_param']['name_size_file'] = Params['KITTI']['name_size_file']
        det_out_param_kitti['save_output_param']['num_test_image'] = Params['KITTI']['num_test_image']
        det_eval_param_kitti['num_classes'] = Params['KITTI']['num_classes']
        det_eval_param_kitti['background_label_id'] = Params['KITTI']['background_label_id']
        det_eval_param_kitti['name_size_file'] = Params['KITTI']['name_size_file']
        if phase == 'deploy':
            det_out_param_kitti['save_output_param'] = {}
        
        net.detection_out_kitti, net.detection_eval_kitti = create_multibox_outputs(
                net, mbox_layers_kitti, 'label_kitti', conf_name='mbox_conf_kitti',
                det_out_param=det_out_param_kitti, 
                det_eval_param=det_eval_param_kitti)
        
        if phase == 'deploy':
            net.detection_out = L.Concat(net.detection_out_kitti, net.detection_out_lisa, axis=1)
    
    if len(garbage_layers) != 0:
        net.garbage = L.Silence(*garbage_layers, ntop=0)

    net_proto = net.to_proto()
    if phase == 'deploy':
        del net_proto.layer[proto_index(net_proto, 'data_lisa')]
        del net_proto.layer[proto_index(net_proto, 'data_kitti')]
        del net_proto.layer[proto_index(net_proto, 'data')]
        del net_proto.layer[proto_index(net_proto, 'detection_eval_lisa')]
        del net_proto.layer[proto_index(net_proto, 'detection_eval_kitti')]
